                    ...
                    <table class="table table-hover">
                        <tbody>
                            ...
                            @foreach ($nodes as $node)
                                <tr>
                                    <td class="text-center text-muted left-icon" data-action="ping"
                                        data-secret="{{ $node->getDecryptedKey() }}"
                                        data-location="{{ $node->scheme }}://{{ $node->fqdn }}:{{ $node->daemonListen }}/api/system">
                                        <i class="fa fa-fw fa-refresh fa-spin"></i>
                                    </td>
                                    <td>{!! $node->maintenance_mode ? '<span class="label label-warning"><i class="fa fa-wrench"></i></span> ' : '' !!}<a
                                    ...
                                    <td class="text-center"
                                        style="color:{{ $node->scheme === 'https' ? '#50af51' : '#d9534f' }}"><i
                                            class="fa fa-{{ $node->scheme === 'https' ? 'lock' : 'unlock' }}"></i></td>
                                    <td class="text-center"><i class="fa fa-{{ $node->public ? 'eye' : 'eye-slash' }}"></i>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
        ...
        (function pingNodes() {
            $('td[data-action="ping"]').each(function(i, element) {
                $.ajax({
                    ...
                    $(element).removeClass('text-muted').find('i').removeClass().addClass(
                    ...
                    $(element).removeClass('text-muted').find('i').removeClass().addClass(
                        'fa fa-fw fa-heart-o').css('color', '#d9534f');
                    $(element).find('i').tooltip({
                        title: errorText
                    });
                });
