@php
    use Pterodactyl\Support\AdminNavigation;

    $nav = AdminNavigation::get();
    $items = AdminNavigation::getMerged();
@endphp
<!DOCTYPE html>
<html class="dark">
<head>
    ...
    <title>{{ config('app.name', 'Reviactyl') }} - @yield('title')</title>
...
<nav class="fixed top-0 left-0 right-0 h-20 flex items-center justify-between bg-zinc-900/50 backdrop-blur-md border-b border-zinc-800 p-3 md:p-4 z-50">
    <a href="{{ route('index') }}" class="flex items-center space-x-2 md:space-x-3 group">
        <div class="flex flex-col">
            <span class="font-bold text-lg md:text-xl bg-gradient-to-r from-orange-400 to-red-400 bg-clip-text text-transparent">{{ config('app.name', 'Reviactyl') }}</span>
            <span class="text-xs text-zinc-400 -mt-1 hidden md:block">Release v{{ config('app.version') }}</span>
        </div>
    </a>
    
    <div class="flex items-center">
        <a href="{{ route('account') }}" class="flex items-center space-x-2 md:space-x-3 px-2 md:px-4 py-1.5 md:py-2 bg-zinc-800/50 hover:bg-zinc-700/50 border border-zinc-700 h-10 md:h-12 rounded-l-lg transition-all duration-200 group">
...
@if(Auth::user()->root_admin)
    <form id="logoutForm" action="{{ route('auth.logout') }}" method="POST" class="hidden">
        @csrf
    </form>
    ...
    <a href="#" 
       ...
       class="group relative flex items-center justify-center h-10 w-10 md:h-12 md:w-12 
              rounded-r-lg bg-red-800/50 border border-red-700 text-red-400 
              hover:bg-red-700/50 hover:text-red-300 transition-all duration-200">
        <i class="fa-solid fa-right-from-bracket"></i>
        <div class="absolute top-full mt-2 px-2 py-1 bg-zinc-800 text-white text-sm rounded 
                    opacity-0 group-hover:opacity-100 transition-opacity duration-200 
                    whitespace-nowrap z-50 pointer-events-none hidden md:block">
            Logout
        </div>
    </a>
@endif
...
<aside class="w-64 bg-zinc-900/30 backdrop-blur-md border-r border-zinc-800 flex flex-col justify-between py-4 md:py-6 flex-shrink-0 z-40 overflow-y-auto">
    <div class="flex flex-col space-y-3 md:space-y-4">
        <nav class="flex flex-col space-y-1 px-3">
            @foreach ($items as $item)
                @if (isset($item['header']))
                    <p class="px-2 py-2 text-xs font-semibold uppercase tracking-wider text-zinc-400">
                        {{ $item['header'] }}
                    </p>
                @elseif(isset($item['route'], $item['label'], $item['icon']))
                    <a href="{{ route($item['route']) }}"
                       class="{{ AdminNavigation::isActive($item) }} group flex items-center gap-3 px-3 py-2 rounded-lg text-orange-500 hover:bg-orange-600/30 hover:text-orange-300 transition-all duration-200">
